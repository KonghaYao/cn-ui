...
export const observeElementOffset = <T extends Element>(instance: Virtualizer<T, any>, cb: (offset: number) => void) => {
    const element = instance.scrollElement
    if (!element) {
        return
    }
...
export interface VirtualizerOptions<TScrollElement extends Element | Window, TItemElement extends Element> {
    ...
    scrollToFn: (offset: number, options: { adjustments?: number; behavior?: ScrollBehavior }, instance: Virtualizer<TScrollElement, TItemElement>) => void
...
export class Virtualizer<TScrollElement extends Element | Window, TItemElement extends Element> {
    ...
    getElementOffset(instance: Virtualizer<TScrollElement, TItemElement>) {
        ...
        if (instance.scrollElement instanceof Window) {
            return Math.abs(instance.scrollElement[instance.options.horizontal ? 'scrollX' : 'scrollY'])
        }
        // 反向列表的数值是负数，所以需要进行正数操作
        return Math.abs(instance.scrollElement[instance.options.horizontal ? 'scrollLeft' : 'scrollTop'])
    }
